...
class ChiTietBangLuongController extends Controller
{
    public function getIndex($MaBangLuong)
    {
        $user = Auth::guard('admin')->user();
        if ($user) {
            $data = ChiTietBangLuong::with('bangluong', 'nhanvien')->where('MaBangLuong', $MaBangLuong)
                ->paginate(10);
            return view('Admin.Chitietbangluong.view', compact('data'));
        }
    ...
    public function formUpdate($id)
    {
        ...
        if ($user) {
            ...
            $data = ChiTietBangLuong::with('nhanvien')->where('MaCTLuong', $id)->first();
            return view('Admin.Chitietbangluong.edit', compact('data'));
        }
        return redirect()->route('admin.login.form');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $data = $request->all();
        $mabangluong = ChiTietBangLuong::where('MaCTLuong', $id)->first();
        $update = ChiTietBangLuong::where('MaCTLuong', $id)->update([  // nếu create thât bại sẽ trả về giá trị null
            'TongPhuCap' => $request->phucap,
            'TongThue' => $request->thue,
            'baohiem' => $request->baohiem,
            'tienthuong' => $request->khenthuong,
            'giamtru' => $request->giamtru,
            'LuongThucLinh' => $request->tongluong,
            'danhan' => $request->datranv

        ]);
        if ($update !== null) {
            return redirect()->route('admin.ctbangluong.form', ['MaBangLuong'=>$mabangluong->MaBangLuong])->with('success', 'Data has been processed successfully.');
        } else {
            return redirect()->back()->with('error', 'Data processing failed. Please try again.');
        }
    }
    ...
    {
        $data = ChiTietBangLuong::where('MaCTLuong', $id)->first();
